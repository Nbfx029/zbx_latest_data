...
if [ "${#IPS[@]}" -eq 0 ] && [ -z "$HOSTGROUP" ]; then
  echo "エラー: 対象を指定してください(IPアドレスを引数か -f ファイルで渡すか、-g でホストグループを指定)。" >&2
  exit 1
fi
...
zbx_call() {
  ...
  if [ "$use_auth" = "with_auth" ]; then
    if [ -n "$ZBX_TOKEN" ]; then
      headers+=(-H "Authorization: Bearer ${ZBX_TOKEN}")
    elif [ -n "${AUTH_ID:-}" ]; then
...
if [ -z "$ZBX_TOKEN" ]; then
  if [ -z "$ZBX_USER" ] || [ -z "$ZBX_PASS" ]; then
    echo "エラー: 認証情報がありません。ZBX_TOKEN か、ZBX_USER/ZBX_PASS のいずれかを設定してください。" >&2
    exit 1
  fi
